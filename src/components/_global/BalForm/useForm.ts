...
export default function useForm<T>({
  ...
  function setValue(
    key: string,
    value: any,
    opts?: { triggerValidation?: boolean }
  ) {
    if (get(formState.values, key) === undefined) {
      throw new Error(
        `[${key}] is not a registered input for form [${formState.name}]`
  ...
  return {
    ...formState,
    values: toRefRecursive(formState.values),
    errors: toRefRecursive(formState.errors)
  } as ReactiveFormInstance<T>;
}
