...
export function encodeJoinStablePool(
  joinData: JoinStablePoolInit | JoinStablePoolAllTokensInForExactBPTOut
): string {
  if (joinData.kind == 'Init') {
    ...
    return defaultAbiCoder.encode(
      ['uint256', 'uint256'],
      [
        JOIN_STABLE_POOL_ALL_TOKENS_IN_FOR_EXACT_BPT_OUT_TAG,
...
export function encodeExitStablePool(
  exitData: ExitStablePoolExactBPTInForAllTokensOut
): string {
  return defaultAbiCoder.encode(
    ['uint256', 'uint256'],
