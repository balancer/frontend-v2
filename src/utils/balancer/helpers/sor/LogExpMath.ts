...
export function n_log(a: BigNumber): BigNumber {
  ...
  const z = PRECISION.times(a.minus(PRECISION)).idiv(a.plus(PRECISION));
  let s = z;
  const z_squared = z.times(z).idiv(PRECISION);
...
export function n_log_36(a: BigNumber): BigNumber {
  a = a.times(DECIMALS);
  const z = DOUBLE_DECIMALS.times(a.minus(DOUBLE_DECIMALS)).idiv(
    a.plus(DOUBLE_DECIMALS)
  );
  let s = z;
  const z_squared = z.times(z).idiv(DOUBLE_DECIMALS);
